...
* **Inventory: Cap tracker** — the same information where you need it while playing, on the Farmhand button and in the briefing panel. The button carries a count on its left shoulder: red = items at cap that drop where you are (every one you find is thrown away), orange = only near. The panel's **Cap** tab shows those as an icon grid (click to open the item), then everything at or near cap regardless of where you are, with counts, and a refresh link. Each row is tagged with who ♥ loves / ♡ likes it (linking to their page), and the townsperson on bonus friendship XP today sits in a **Daily friend** card at the top with a ★ on any tag that's theirs — the capped stack you should hand over first.
* **Craftworks: Queue plan** — the queue the panel would build for what you're doing: your focused undertakings (or everything, with no focus), and when you're at an explore or fishing spot, the slice of that whose ingredients drop there. Each row is marked queued / not queued against the live slots. **Save as set** stores the list as a Craftworks set in one request (the game's own share-page action — the live queue is untouched) and it then loads through the usual confirmed, restorable loader. Nothing is added to the queue for you.
  ...
  * Each at-cap row says who **loves** (♥) or **likes** (♡) the item, linking to that townsperson's page — a full stack becomes a gift instead of a discarded drop.

#### The briefing panel

* **Briefing: Floating panel** — the Farmhand button above the bottom bar opens a panel organised by the question you came with: **Now** (what wants you this minute — turn-ins, one-item-away requests, stalled Craftworks slots), **Here** (the drop table of the place you're standing, with your count against the cap, who wants each drop, silver and XP per explore, stamina banked, and "wanted here" / "thrown away here" verdicts on top — in a mine, where no site has a drop table, the same view built from what its dig board has turned up so far — or where to go, when you're nowhere in particular), **Goals**, **Craftworks** (queue advice and saved sets), and **Cap**.
* **Search (Ctrl+K)** — buddy.farm inside the game. Type an item, quest, townsperson or location and it opens in the panel as buddy.farm's page with the game's own links and your numbers on it: where an item drops (1 in N, and how many tries for what you need), its recipe against your inventory, what it's used in, the quests that need it (linking straight to the quest), who loves it, tower floors and exchange trades. Quests show requirements against what you hold ("3 short"); townsfolk show what you have in hand to give them. Every row's ▸ opens the next thing in the panel, so item → quest → item never leaves it. `q:`, `i:`, `t:`, `l:` narrow to a kind.

#### Perk sets that are actually equipped when it counts

* **Perk set indicator** — a coloured dot and the set's name in the bottom stats bar, just after your currency counts, showing which set you're playing under: green once the game has confirmed it equipped, amber (hollow) while a switch is in flight, grey when it's only what was last requested. It reports what the game has confirmed equipped, not merely what was requested. It replaces upstream's "…perks activated" banners, which pushed the page (and whatever button was under your finger) down each time they appeared.
